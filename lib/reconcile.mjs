...
export function reconcile(roster, mirrors = [], cfg = {}) {
  // HOLD only on a transient herdr read failure (null/non-array). A healthy
  // empty roster is authoritative: it means every previously mirrored agent is gone,
  // so the stale-row pass below must be allowed to close or gray those mirrors.
