...
// is always reconstructable from cmux itself (the herdr key is stored in each
// mirror workspace's HPCX_KEY env var). map.json just caches last-seen status so
...
export function loadState(env = process.env) {
  try {
    ...
    return {
      byKey: d.byKey || {},
      suppress: new Set(d.suppress || []),
      foreign: new Set(d.foreign || []), // workspace ids confirmed not ours (skip env probes)
    };
  } catch {
    return { byKey: {}, suppress: new Set(), foreign: new Set() };
  }
}
